...
public static class GameSceneCameraLayerUtility
{
    ...
    /// /params environmentMask Layers already rendered by the post-processed environment base camera.
    /// /params additionalExcludedMask Extra layers that should not render in the gameplay overlay camera.
    /// /returns Derived culling mask for the gameplay overlay camera.
    /// </summary>
    ...
    /// /params firstMask First Unity layer mask.
    /// /params secondMask Second Unity layer mask.
    /// /returns True when the masks overlap.
    /// </summary>
    ...
    /// /params layerName Unity layer name to resolve.
    /// /params layerMask Mask containing the resolved layer when available.
    /// /returns True when the layer name exists in Project Settings.
    /// </summary>
