...
/// <summary>
/// Provides a UI panel for creating, editing, duplicating and deleting player UI visual presets.
/// </summary>
public sealed class PlayerUiVisualPresetsPanel : IPlayerVisualPresetEditorPanel
{
    #region Constants
    private const float LeftPaneWidth = 280f;
    private const string SelectedPresetPathStateKey = "NashCore.PlayerManagement.UiVisual.SelectedPreset";
    private const string ActiveSectionStateKey = "NashCore.PlayerManagement.UiVisual.ActiveSection";
    private const string ActiveSubSectionStateKey = "NashCore.PlayerManagement.UiVisual.ActiveSubSection";
    private const string DetailsScrollOffsetStateKey = "NashCore.PlayerManagement.UiVisual.DetailsScroll";
    #endregion

    #region Fields
    private readonly VisualElement root;
    private readonly List<PlayerUiVisualPreset> filteredPresets = new List<PlayerUiVisualPreset>();
    ...
    private PlayerUiVisualPresetLibrary library;
    ...
    private PlayerUiVisualPreset selectedPreset;
    private SerializedObject presetSerializedObject;
    private SectionType activeSection = SectionType.Metadata;
    private UiVisualSubSectionType activeUiVisualSubSection = UiVisualSubSectionType.HealthBars;
    #endregion
    ...
    #region Constructors
    /// <summary>
    /// Initializes the player UI visual preset panel and restores the previously selected sections.
    /// </summary>
    public PlayerUiVisualPresetsPanel()
    {
        root = new VisualElement();
        root.style.flexGrow = 1f;

        library = PlayerUiVisualPresetLibraryUtility.GetOrCreateLibrary();
        activeSection = ManagementToolStateUtility.LoadEnumValue(ActiveSectionStateKey, SectionType.Metadata);
        activeUiVisualSubSection = ManagementToolStateUtility.LoadEnumValue(ActiveSubSectionStateKey, UiVisualSubSectionType.HealthBars);
    ...
    {
        PlayerUiVisualPreset previouslySelectedPreset = selectedPreset;
        library = PlayerUiVisualPresetLibraryUtility.GetOrCreateLibrary();
        RefreshPresetList();
    ...
    public void SelectPresetFromExternal(PlayerUiVisualPreset preset)
    {
        if (preset == null)
            return;
    ...
    /// <summary>
    /// Builds the searchable preset list and asset action toolbar.
    /// </summary>
    ...
    /// <summary>
    /// Builds the details scroll view and attaches persistent scroll state.
    /// </summary>
    ...
    private VisualElement MakePresetItem()
    {
        ...
        {
            PlayerUiVisualPreset preset = label.userData as PlayerUiVisualPreset;
    ...
    private void BindPresetItem(VisualElement element, int index)
    {
        ...
        PlayerUiVisualPreset preset = filteredPresets[index];
    ...
    private void OnPresetSelectionChanged(IEnumerable<object> selection)
    {
        ...
        {
            PlayerUiVisualPreset preset = item as PlayerUiVisualPreset;
    ...
    internal void RefreshPresetList()
    {
        ...
        if (library != null)
        {
            ...
            {
                PlayerUiVisualPreset preset = library.Presets[index];
        ...
        {
            PlayerUiVisualPreset restoredPreset = ManagementToolStateUtility.LoadAsset<PlayerUiVisualPreset>(SelectedPresetPathStateKey);
            PlayerUiVisualPreset initialPreset = restoredPreset != null && filteredPresets.Contains(restoredPreset)
                ? restoredPreset
                : filteredPresets[0];
    ...
    private static bool IsMatchingSearch(PlayerUiVisualPreset preset, string searchText)
    {
    ...
    /// <summary>
    /// Creates and selects one new UI visual preset asset.
    /// </summary>
    private void CreatePreset()
    {
        PlayerUiVisualPreset newPreset = PlayerUiVisualPresetLibraryUtility.CreatePresetAsset("PlayerUiVisualPreset");

        if (newPreset == null)
            return;

        Undo.RegisterCreatedObjectUndo(newPreset, "Create Player UI Visual Preset Asset");
        Undo.RecordObject(library, "Add Player UI Visual Preset");
    ...
    private void DuplicatePreset(PlayerUiVisualPreset preset)
    {
        if (preset == null)
            return;

        PlayerUiVisualPreset duplicatedPreset = ScriptableObject.CreateInstance<PlayerUiVisualPreset>();
        ...
        if (string.IsNullOrWhiteSpace(duplicateBaseName))
            duplicateBaseName = "PlayerUiVisualPreset Copy";
        ...
        Undo.RegisterCreatedObjectUndo(duplicatedPreset, "Duplicate Player UI Visual Preset Asset");
        ...
        Undo.RecordObject(library, "Duplicate Player UI Visual Preset");
    ...
    private void DeletePreset(PlayerUiVisualPreset preset)
    {
        if (preset == null)
            return;

        bool confirmed = EditorUtility.DisplayDialog("Delete Player UI Visual Preset", "Delete the selected preset asset?", "Delete", "Cancel");

        if (!confirmed)
            return;

        Undo.RecordObject(library, "Delete Player UI Visual Preset");
    ...
    private void SelectPreset(PlayerUiVisualPreset preset)
    {
    ...
    private void BuildActiveDetailsSection()
    {
        ...
        switch (activeSection)
        {
            case SectionType.Metadata:
                PlayerUiVisualPresetsPanelSectionsUtility.BuildMetadataSection(this);
                break;

            case SectionType.Visual:
                PlayerUiVisualPresetsPanelSectionsUtility.BuildVisualSection(this);
                break;
        }
    ...
    internal void RegeneratePresetId()
    {
        ...
        Undo.RecordObject(selectedPreset, "Regenerate Player UI Visual Preset ID");
        presetSerializedObject.Update();
        idProperty.stringValue = Guid.NewGuid().ToString("N");
    ...
    private void RenamePreset(PlayerUiVisualPreset preset, string newName)
    {
        if (preset == null)
            return;
    ...
    /// <param name="preset">Preset to rename.</param>
    private void ShowRenamePopup(VisualElement anchor, PlayerUiVisualPreset preset)
    {
        ...
        PresetRenamePopup.Show(anchorRect,
                               "Rename Player UI Visual Preset",
                               preset.PresetName,
    ...
    private static string GetPresetDisplayName(PlayerUiVisualPreset preset)
    {
        if (preset == null)
    ...
    {
        PlayerUiVisualPresetsPanelSectionsUtility.ShowActiveUiVisualSubSection(this);
    }

    /// <summary>
    /// Rebuilds the current details section after serialized array changes.
    /// </summary>
